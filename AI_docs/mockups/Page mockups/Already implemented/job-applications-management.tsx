...
import { Leaf, Search, Filter, MoreVertical, Eye, Calendar, Mail, Phone, User, MapPin, Clock, Building2, Briefcase, GraduationCap, Star, ChevronDown, ArrowLeft, Bell, Settings, FileText, Download } from 'lucide-react';
...
const JobApplicationsManagement = () => {
  ...
  const applicants = [
    {
      ...
      skills: ['Social Media Marketing', 'Content Creation', 'Adobe Creative Suite', 'Google Analytics', 'Email Marketing', 'Brand Strategy', 'Copywriting', 'Photoshop', 'Video Editing', 'SEO Optimization'],
      resumeUrl: '/resumes/sarah-chen.pdf',
      coverLetterExcerpt: 'I\'m excited to bring my creative marketing experience to TechFlow Solutions...',
      matchScore: 92,
      volunteerWork: [
    ...
    },
    {
      ...
      skills: ['Campaign Management', 'Data Analysis', 'Brand Strategy', 'SEO/SEM', 'Marketing Automation', 'A/B Testing', 'Customer Segmentation', 'CRM Management', 'Lead Generation', 'Conversion Optimization', 'Google Ads', 'Facebook Ads'],
      resumeUrl: '/resumes/marcus-rodriguez.pdf',
      coverLetterExcerpt: 'With my extensive background in digital marketing campaigns...',
      matchScore: 88,
      volunteerWork: [
    ...
    },
    {
      ...
      skills: ['Email Marketing', 'Social Media Management', 'Event Planning', 'Content Writing', 'Customer Service', 'Project Management', 'Canva Design', 'WordPress', 'Mailchimp', 'Hootsuite', 'Google Workspace'],
      resumeUrl: '/resumes/emily-thompson.pdf',
      coverLetterExcerpt: 'I would love to contribute my enthusiasm and fresh perspective...',
      matchScore: 85,
      volunteerWork: [
    ...
    },
    {
      ...
      skills: ['Project Management', 'Digital Marketing', 'Team Collaboration', 'Presentation Skills', 'Agile Methodology', 'Stakeholder Management', 'Budget Planning', 'Market Research', 'Content Strategy', 'Cross-functional Leadership'],
      resumeUrl: '/resumes/david-kim.pdf',
      coverLetterExcerpt: 'I\'m particularly drawn to TechFlow\'s innovative approach...',
      matchScore: 90,
      volunteerWork: [
    ...
    },
    {
      ...
      skills: ['Social Media', 'Basic Analytics', 'Customer Service', 'Microsoft Office', 'Entry-level Design', 'Communication'],
      resumeUrl: '/resumes/jessica-miller.pdf',
      coverLetterExcerpt: 'Although I\'m early in my career, I\'m eager to learn...',
      matchScore: 72,
      volunteerWork: [
  ...
  const statusOptions = [
    ...
    { value: 'unreviewed', label: 'Unreviewed', count: applicants.filter(a => a.status === 'unreviewed').length },
    { value: 'reviewed', label: 'Reviewed', count: applicants.filter(a => a.status === 'reviewed').length },
    { value: 'shortlisted', label: 'Shortlisted', count: applicants.filter(a => a.status === 'shortlisted').length },
    { value: 'interview_scheduled', label: 'Interview Scheduled', count: applicants.filter(a => a.status === 'interview_scheduled').length },
    { value: 'rejected', label: 'Rejected', count: applicants.filter(a => a.status === 'rejected').length }
  ];

  const getStatusBadge = (status) => {
    const statusConfig = {
      unreviewed: { bg: 'bg-yellow-100', text: 'text-yellow-800', label: 'Unreviewed' },
  ...
  const getStatusDropdownOptions = (currentStatus) => {
    const allStatuses = [
    ...
    // Don't show current status in dropdown options
    return allStatuses.filter(status => status.value !== currentStatus);
  };

  const StatusDropdown = ({ applicant }) => {
    const isOpen = openStatusDropdowns.has(applicant.id);
    const statusConfig = getStatusBadge(applicant.status);
    const dropdownOptions = getStatusDropdownOptions(applicant.status);
    
    return (
      <div className="relative">
        {/* Status Badge Button - enhanced hover effects to show it's clickable */}
        <button
        ...
        {/* Dropdown Menu - simplified without descriptions */}
        {isOpen && (
          <>
              ...
              <div className="py-2">
                ...
                {dropdownOptions.map((option) => {
                  ...
                  return (
                    <button
                      key={option.value}
                      onClick={() => updateApplicantStatus(applicant.id, option.value)}
                      className="w-full text-left px-3 py-2 hover:bg-green-50 transition-colors flex items-center justify-center"
                    >
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${optionConfig.bg} ${optionConfig.text}`}>
                        {optionConfig.label}
                      </span>
                    </button>
                  );
  ...
  const toggleCardExpansion = (applicantId) => {
  ...
  const toggleStatusDropdown = (applicantId) => {
  ...
  const updateApplicantStatus = (applicantId, newStatus) => {
    ...
    // TODO: Update applicant status in state/backend
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-3">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              ...
              <button className="text-green-700 hover:text-green-800 font-medium transition-colors">Job Listings</button>
              <button className="text-green-800 font-semibold">Applications</button>
              <button className="text-green-700 hover:text-green-800 font-medium transition-colors">Company Profile</button>
            </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-xl shadow-lg border border-green-200 p-4 sm:p-6 mb-8">
          ...
          <div className="flex items-center space-x-2 mb-4">
            ...
            <span className="text-sm text-green-600">Applications shown below are for this position</span>
          </div>
          
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between space-y-4 lg:space-y-0">
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-green-900 mb-2 break-words">{jobInfo.title}</h1>
              ...
              <p className="text-green-600 text-sm">Posted {jobInfo.posted} • {applicants.length} applications received</p>
            </div>
        ...
        <div className="space-y-6">
          {filteredApplicants.map((applicant) => {
            ...
            // Smart truncation limits - Progressive disclosure strategy
            // Default: Show minimal info for quick scanning
            // Expanded: Show more detail but still manageable
            // Full Application: Complete details (separate page/modal)
            ...
            return (
              <div key={applicant.id} className="bg-white/70 backdrop-blur-sm border border-green-200 rounded-xl shadow-sm hover:shadow-md transition-all p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-start space-y-4 sm:space-y-0 sm:space-x-4">
                  {/* Mobile: Top row with checkbox, avatar, and basic info */}
                    ...
                    <div className="flex-1 min-w-0 sm:hidden">
                      <div className="flex items-start justify-between">
                        <div className="min-w-0">
                          <h3 className="text-lg font-semibold text-green-900 truncate">{applicant.name}</h3>
                          <p className="text-green-700 font-medium text-sm truncate">{applicant.currentRole}</p>
                        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="hidden sm:flex sm:items-start sm:justify-between mb-3">
                      <div className="min-w-0 flex-1">
                        <h3 className="text-xl font-semibold text-green-900 mb-1 break-words">{applicant.name}</h3>
                        <p className="text-green-700 font-medium mb-1 break-words">{applicant.currentRole}</p>
                      ...
                      <div className="flex items-center space-x-3 ml-4">
                        ...
                        <div className="text-right">
                          <div className="flex items-center space-x-1 text-amber-600">
                            ...
                            <span className="font-semibold text-sm">{applicant.matchScore}%</span>
                          </div>
                    ...
                    <div className="sm:hidden space-y-3 mb-4">
                      ...
                      <p className="text-xs text-green-700 truncate">{applicant.education}</p>
                    </div>

                    {/* Contact Info - responsive */}
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-xs sm:text-sm text-green-600 mb-3">
                      <a href={`mailto:${applicant.email}`} className="flex items-center hover:text-green-800 truncate">
                        <Mail className="w-3 h-3 mr-1 flex-shrink-0" />
                        <span className="truncate">{applicant.email}</span>
                      </a>
                      <a href={`tel:${applicant.phone}`} className="flex items-center hover:text-green-800 whitespace-nowrap">
                    ...
                    {/* Skills - with smart truncation */}
                    <div className="mb-3">
                      <h4 className="text-sm font-medium text-green-800 mb-2">Key Skills:</h4>
                      <div className="flex flex-wrap gap-1 sm:gap-2">
                        {/* Show limited skills in collapsed view, all skills when expanded */}
                        {applicant.skills
                          .slice(0, isExpanded ? applicant.skills.length : defaultSkillsLimit)
                          .map((skill, index) => (
                            <span key={index} className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">
                              {skill}
                            </span>
                          ))}
                        {/* Show "+X more" indicator when collapsed and there are more items */}
                        {!isExpanded && applicant.skills.length > defaultSkillsLimit && (
                          <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-full text-xs">
                            +{applicant.skills.length - defaultSkillsLimit} more
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="mb-4">
                      <h4 className="text-sm font-medium text-green-800 mb-1">Cover Letter Preview:</h4>
                      <p className="text-sm text-green-700 italic break-words">&quot;{applicant.coverLetterExcerpt}&quot;</p>
                    </div>

                    {/* Volunteer Work & Personal Projects - responsive grid with smart truncation */}
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                      {/* Volunteer Work - Progressive disclosure: 2 → 5 → "View full application" */}
                      <div>
                        <h4 className="text-sm font-medium text-green-800 mb-1">Volunteer Experience:</h4>
                        <ul className="text-sm text-green-700 space-y-1">
                          {applicant.volunteerWork
                            .slice(0, isExpanded ? expandedVolunteerLimit : defaultVolunteerLimit)
                            .map((work, index) => (
                          ...
                          {/* Collapsed state: show "+X more" for items beyond default limit */}
                          {!isExpanded && applicant.volunteerWork.length > defaultVolunteerLimit && (
                            <li className="flex items-start text-green-500">
                              ...
                              <span className="text-xs">+{applicant.volunteerWork.length - defaultVolunteerLimit} more volunteer experiences</span>
                            </li>
                          )}
                          {/* Expanded state: if still more beyond expanded limit, guide to full application */}
                          {isExpanded && applicant.volunteerWork.length > expandedVolunteerLimit && (
                            <li className="flex items-start text-green-500">
                              <span className="w-1 h-1 bg-green-400 rounded-full mt-2 mr-2 flex-shrink-0"></span>
                              <span className="text-xs">View full application for {applicant.volunteerWork.length - expandedVolunteerLimit} more</span>
                            </li>
                          )}
                        </ul>
                      </div>
                      
                      {/* Personal Projects - Same progressive disclosure strategy as volunteer work */}
                      <div>
                        <h4 className="text-sm font-medium text-green-800 mb-1">Personal Projects:</h4>
                        <ul className="text-sm text-green-700 space-y-1">
                          {applicant.personalProjects
                            .slice(0, isExpanded ? expandedProjectsLimit : defaultProjectsLimit)
                            .map((project, index) => (
                              <li key={index} className="flex items-start">
                          ...
                          {!isExpanded && applicant.personalProjects.length > defaultProjectsLimit && (
                            <li className="flex items-start text-amber-600">
                              ...
                              <span className="text-xs">+{applicant.personalProjects.length - defaultProjectsLimit} more projects</span>
                            </li>
                          )}
                          {isExpanded && applicant.personalProjects.length > expandedProjectsLimit && (
                            <li className="flex items-start text-amber-600">
                              <span className="w-1 h-1 bg-amber-400 rounded-full mt-2 mr-2 flex-shrink-0"></span>
                              <span className="text-xs">View full application for {applicant.personalProjects.length - expandedProjectsLimit} more</span>
                            </li>
                          )}
                        </ul>
                      </div>
        ...
        {filteredApplicants.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-medium text-green-800 mb-2">No applications found</h3>
